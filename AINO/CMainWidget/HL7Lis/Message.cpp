...
Message::Message()
{
    encoding=HL7Encoding::getInstance();
}
...
void Message::addSegmentMSH(std::string sendingApplication, std::string sendingFacility,
                            std::string receivingApplication, std::string receivingFacility,
                            std::string security, std::string messageType,
                            std::string messageControlID, std::string processingID,
                            std::string version, ResultType resultType)
{
    ...
    std::string response = "MSH" + this->encoding->allDelimiter() + delim +
                           ...
                           processingID + delim +
                           version + delim + delim + delim + delim;

    if (resultType == rtNone)
    {
        response = response + delim+delim+"ASCLL" +delim+delim +delim+
                           this->encoding->_segmentDelimiter;
    }else
    {
        char rtChar = '0';
        if (resultType == rtQC)
        ...
        response = response + rtChar + delim+delim+"ASCLL" +delim+delim +delim+
                   this->encoding->_segmentDelimiter;
